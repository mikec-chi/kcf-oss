...
|---|---|---|
| `concept` **ENTITY** + `attribute` (identity/required/optional/type/default) | ✅ table/model + CRUD; `identity`→PK, `required`→NOT NULL, `default`→column default | ✅ list + detail + form; `type`→input widget, `required`→validation, `identity`→read-only key. **Labels are `humanize()`d** from identifiers (Title-Case, acronyms upper-cased — never raw `firstName`; see system-prompt rule 12). The **list view** follows the data-grid convention (rule 13): identity/UUID and free-text columns dropped, sortable headers, a filter bar = free-text search + faceted filters for categorical columns (enum/boolean/lifecycle-state), paginated. |
...
- `category master|transactional|reference|config` → the data-management role, a
  ...
  (Master data / Transactions / Configuration) instead of a flat entity list, and
  **sub-group the Transactions by `process`** — the business flow whose works
  `TRANSFORMATION`-transform each record (see COOKBOOK §F).
...
| `experience` | ⬚ **out-of-tier** (frontend owns UX) | ✅ **primary driver**: screens, flows, navigation. **Nav = aggregate roots only**; pure parts (COMPOSITION targets with no children and no independent inbound ref) render as **subtabs** on their parent's detail (parent = the COMPOSITION source), not as their own nav entry; **transactional entities are sub-grouped by the `process`** whose works TRANSFORMATION-transform them — see COOKBOOK §F. |
| `design` | ⬚ **out-of-tier** | ✅ theming / design tokens / component styling via a selectable **design-system preset** (`design-systems/README.md`); default = `design-system-default.md` (brand-neutral, accessible baseline). A preset is the *skin only* — structure stays IR-derived and identical across presets; model `design` tokens override the preset's brand values. |
