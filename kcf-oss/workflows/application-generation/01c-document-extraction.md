# 01c - Document Extraction (flowchart / org chart / form)

## Gate

Used when the input is a structured or visual document rather than prose. The
document's *structure* is its content, and each document type targets a specific
dimension. This step extracts it with the same traceability as 01b, guided by the
document profile for its modality. KCF does not read images - an LLM (or a
...
```text
Input: [DOCUMENT_PATH] of kind [DOCUMENT_KIND] (flowchart | org-chart | form | ...).
